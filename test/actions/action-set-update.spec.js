...
const {
	handler
} = require('../../lib/actions/action-set-update')
...
ava('handler() should update array when property path is an array', async (test) => {
...
ava('handler() should update array when property path is a string', async (test) => {
	const msg = _.cloneDeep(message)
	msg.id = uuid.v4()
	const context = makeContext([
		msg
