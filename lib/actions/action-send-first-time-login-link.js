...
const addFirstTimeLogin = async ({
	...
	const sevenDaysinFuture = requestedAt.setHours(requestedAt.getHours() + (24 * 7))
...
const handler = async (session, context, userCard, request) => {
	...
	assert.USER(request.context, userCard.data.email && userCard.data.email.length, context.errors.WorkerNoElement,
