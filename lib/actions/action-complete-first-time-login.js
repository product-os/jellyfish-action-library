...
const pre = require('./action-complete-password-reset').pre
...
const handler = async (session, context, card, request) => {
	...
	if (!firstTimeLogin) {
		const error = new context.errors.WorkerAuthenticationError('First-time login token invalid')
		error.expected = true
		logger.warn(request.context, `Could not find firstTimeLogin card with token ${request.arguments.firstTimeLoginToken}`)
		throw error
	}
	...
	if (!user) {
		...
		logger.warn(request.context, `FirstTimeLogin card with token
	...
	// TODO: Fix this. Is false when data.hash is undefined/null as well.
