...
const pre = async (session, context, request) => {
	...
	if (request.arguments.scope) {
		assert.USER(request.context, skhema.isValid(request.arguments.scope, {}, {
			schemaOnly: true
		}), context.errors.WorkerSchemaMismatch, 'Invalid schema for session scope')
	} else {
...
const handler = async (session, context, card, request) => {
	...
	// TODO: Should generate using jsonwebtoken library?
	const authenticationToken = await uuid.random()
	...
	const result = await context.insertCard(
		context.privilegedSession, sessionTypeCard, {
			...
			data: {
				...
				token: {
					authentication: await bcrypt.hash(authenticationToken, BCRYPT_SALT_ROUNDS)
				}
			}
		})
